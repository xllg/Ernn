...
# Defaults
DATA_DIR = os.path.join(ErnnReader_DATA, 'datasets')
MODEL_DIR = os.path.join(ErnnReader_DATA, 'outputs')
...
def add_train_args(parser):
    ...
    """
    parser.register('type', 'bool', str2bool) # 注册一个boolean类型参数类型
    ...
    runtime.add_argument('--num-epochs', type=int, default=4,
...
def set_defaults(args):
    ...
    if not args.model_name:
        import uuid
        import time
        args.model_name = time.strftime("%Y%m%d-") + str(uuid.uuid4())[:8] # 生成随机UUID
...
def validate_official(args, data_loader, model, gloable_stats, offsets, texts, answers):
    ...
    :param offsets: The character start/end indices for the tokens in each context.
    :param texts: Map of qid --> raw text of examples context (matches offsets).
    :param answers: Map of qid --> list of accepted answers.
    """
    eval_time = utils.Timer()
    ...
    for ex in data_loader:
        ...
        for i in range(batch_size):
            ...
            exact_match.update(utils.metric_max_over_ground_truths(utils.exact_match_score, prediction, ground_truths))
            f1.update(utils.metric_max_over_ground_truths(utils.f1_score, prediction, ground_truths))
        examples += batch_size
    logger.info('dev valid official: Epoch = %d | EM = %.2f | ' %
                (gloable_stats['epoch'], exact_match.avg * 100) +
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if result[args.valid_metric] > stats['best_valid']:
            logger.info('Best valid: %s = %.2f (epoch %d, %d updates)' %
                        (args.valid_metric, result[args.valid_metric], stats['epoch'], model.updates))
...
if __name__=='__main__': # 如果模块是被直接运行的，则代码块被执行，如果模块是被导入的，则代码块不被运行
    parser = argparse.ArgumentParser( # 创建解析器
        'ErnnReader', # 程序的名字
        formatter_class=argparse.ArgumentDefaultsHelpFormatter # 自动将有关默认值的信息添加到每个参数帮助消息中
    )
