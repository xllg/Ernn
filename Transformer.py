...
def subsequent_mask(size):
    # Make out subsequent position
    attn_shape = (1, size, size)
    subsequent_mask = np.triu(np.ones(attn_shape), k=1).astype('uint8')
    return torch.from_numpy(subsequent_mask) == 0

# plt.figure(figsize=(5,5))
# plt.imshow(subsequent_mask(20)[0].tolist())
# plt.show()

def attention(query, key, value, mask=None, dropout=0.0):
    # Compute Scale Dot Product Attention
    d_k =  query.size(-1)
    scores = torch.matmul(query, key.transpose(-2,-2)) / math.sqrt(d_k)
    if mask is not None:
...
class DocReader(nn.Module):
    ...
    def forward(self, src, src_mask):
        output = self.reader(self.src_embed(src), src_mask)
        return output

def make_enmodel(src_vocab, N=6, d_model=300, d_ff=2048, h=6, dropout=0.1):
    # Construct a model object based on hyperparameters.
    c = copy.deepcopy
    attn = MultiHeadedAttention(h, d_model, dropout)
    ff = PositionwiseFeedForward(d_model, d_ff, dropout)
